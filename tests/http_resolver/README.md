...
- The SDK does not follow redirects by default. Delegating to `urllib.request` as `http_resolver_example_impl.py`'s examples do gives you redirect handling for free.
...
The [`test_http_resolver_debug.py`](./test_http_resolver_debug.py) uses `DebugHttpResolver`: logs the method and URL of each request and the status of each response.
